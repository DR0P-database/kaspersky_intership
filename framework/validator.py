...
class ConfigValidator:
    ...
    RFC_3066_REGEX = re.compile(
        r"^[a-zA-Z]{1,3}(_[a-zA-Z0-9]{1,3})?(\.[a-zA-Z0-9-]+)?$"
    )

    REQUIRED_SECTIONS = ["General", "Watchdog"]
    REQUIRED_PARAMS_GENERAL = [
        "ScanMemoryLimit",
        "PackageType",
        "ExecArgMax",
        "AdditionalDNSLookup",
        ...
        for section, params in self.config.items():
            ...
            for key in params:
                if section in expected and key not in expected[section]:
                    raise ValueError(
                        f"{section}.{key} не должен находиться в этой секции"
                    )
    ...
    def _validate_general(self):
        ...
        for param in self.REQUIRED_PARAMS_GENERAL:
            if param not in self.config[section]:
                raise ValueError(
                    f"{section}.{param} обязательный параметр, отсутствует"
                )
        ...
        if path and (not os.path.isabs(path) or not os.path.isdir(path)):
            raise ValueError(
                f"{section}.CoreDumpsPath должен быть существующим абсолютным путем к директории"
            )
        ...
        if locale and not self._is_valid_locale(locale):
            raise ValueError(
                f"{section}.Locale должен соответствовать формату RFC 3066"
            )
    ...
    def _validate_watchdog(self):
        ...
        if not value or not re.fullmatch(r"\d+m", value):
            raise ValueError(
                f'{section}.ConnectTimeout некорректный формат. "<число>m", например "60m"'
            )
        try:
            num = int(value[:-1])
        except ValueError:
            raise ValueError(
                f'{section}.ConnectTimeout значение до "m" должно быть целым числом'
            )
        if not (1 <= num <= 120):
            raise ValueError(
                f'{section}.ConnectTimeout должен быть целым числом от 1 до 120 с суффиксом "m"'
            )
    ...
    def _get_value(self, section, key):
        ...
        if value is None:
            raise TypeError(f"{section}.{key} должен иметь значение не None")
    ...
    def _check_int_range(self, section, key, min_val, max_val):
        ...
        if not value.isdigit() or not (min_val <= int(value) <= max_val):
            raise ValueError(
                f"{section}.{key} должен быть целым числом в интервале [{min_val}-{max_val}]"
            )
    ...
    def _check_bool(self, section, key):
        ...
        if value not in self.BOOL_VALUES:
            raise ValueError(
                f"{section}.{key} должен быть одним из значений {self.BOOL_VALUES}"
            )
    ...
    def _check_memory_value(self, section, key):
        ...
        try:
            ...
            raise ValueError(
                f'{section}.{key} должен быть "off", "auto", или вещественным числом в интервале (0, 100]'
            )
